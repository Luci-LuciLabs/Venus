...
namespace venus {
	namespace {  // ANONYMOUS NAMESPACE BEGIN
		...
		const std::vector<const char *> VALIDATION_LAYERS = {
			...
			"VK_LAYER_LUNARG_crash_diagnostic",  // This extension will show an error for disabled device extensions that are vendor specific, I believe we can ignore it.
...
#else
			VN_LOG_INFO("Validation Layers are currently disabled.");
			return SUPPORT_CHECK_FAILURE;
#endif
		}
	...
	void Instance::setupLayerDebugger() {
		...
		if(vkCreateDebugUtilsMessengerEXT(m_instance, &createInfo, nullptr, &m_debugMessenger) != VK_SUCCESS) {
			VN_LOG_ERROR("Failed to create validation-layer debug-messenger.");
		}
	}
#endif
}  // namespace venus